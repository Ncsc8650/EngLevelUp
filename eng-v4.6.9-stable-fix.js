(() => {
  ...
  function style() {
    let s = $("engV469StableStyle");
    if (!s) { s = document.createElement("style"); s.id = "engV469StableStyle"; document.head.appendChild(s); }
    s.textContent = `
  ...
  function labels() {
    ...
    const text = $("studentAccessText"); if (text) text.textContent = "หลังล็อกอิน ลูกค้าเห็นเฉพาะคำศัพท์ 10 คำในแถวเดียว โชว์เฉพาะคำศัพท์เท่านั้น";
  ...
  function renderStudentWords() {
    ...
    if (current && current.dataset.words === signature && current.children.length === WORD_COUNT) return;
  ...
  function wireAuth() {
    ...
    if (login && login.dataset.v469 !== "1") {
      login.dataset.v469 = "1";
    ...
    const form = $("userForm");
    if (form && form.dataset.v469 !== "1") {
      form.dataset.v469 = "1";
      form.addEventListener("submit", async e => {
        e.preventDefault(); e.stopImmediatePropagation();
        const roleValue = val("newRole") || "student", username = val("newUsername").toLowerCase(), password = val("newPassword"), firstName = val("newFirst"), lastName = val("newLast"), startDate = val("newStart"), endDate = val("newEnd");
        const fail = msg => { const f = $("userFormFeedback"); if (f) f.textContent = msg; toast(msg, "error"); };
        if (!username || !password || !firstName || !lastName || !startDate || !endDate) return fail("สร้างยูสเซอร์ไม่สำเร็จ: กรุณากรอกข้อมูลให้ครบ");
        ...
        const users = getJson("eng_users", []);
        if (users.some(u => String(u.username || "").toLowerCase() === username)) return fail("สร้างยูสเซอร์ไม่สำเร็จ: Username นี้มีอยู่แล้ว");
        users.push({ id: `user_${Date.now()}`, role: roleValue, prefix: val("newPrefix"), firstName, lastName, nickname: val("newNick"), username, passwordHash: await sha256(password), startDate, endDate, status: "active" });
        setJson("eng_users", users);
        addLog("admin_create_user", { username, role: roleValue });
        form.reset();
        toast(`สร้างยูสเซอร์สำเร็จ: ${username}`, "ok");
      }, true);
    }
  }
  ...
  function scoreSpeech(spoken, target) {
    ...
    if (!a || !b) return 0;
    if (a === b) return 100;
    const lettersA = a.replace(/ /g, ""), lettersB = b.replace(/ /g, "");
    let hits = 0;
    for (const ch of lettersB) if (lettersA.includes(ch)) hits += 1;
    return Math.max(20, Math.min(98, Math.round((hits / Math.max(1, lettersB.length)) * 100)));
  }
  function startMic(action, button) {
    const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    const panel = button.closest(".tool-panel") || document;
    const target = action.includes("prompt") || action.includes("speech") ? (panel.querySelector("p")?.textContent || "") : (panel.querySelector("h3")?.textContent || "");
    ...
    const feedback = $(id);
    if (!Recognition) { const msg = "ไมค์ไม่ทำงาน: กรุณาใช้ Chrome/Safari และเปิดเว็บผ่าน https ไม่ใช่ LINE browser"; if (feedback) feedback.textContent = msg; toast(msg, "error"); return; }
    ...
    rec.onerror = e => {
      const msg = e.error === "not-allowed" ? "ไมค์ไม่ทำงาน: กรุณากดอนุญาต Microphone" : `ไมค์ไม่ทำงาน: ${e.error || "ลองใหม่อีกครั้ง"}`;
      if (feedback) feedback.textContent = msg;
      toast(msg, "error");
    };
    rec.onend = () => { if (feedback && feedback.textContent.includes("กำลังฟัง")) feedback.textContent = "ไม่ได้ยินเสียง กรุณากดปุ่มพูดตอบแล้วลองใหม่"; };
    try { rec.start(); toast("เริ่มฟังเสียงแล้ว กรุณาพูดตอบ", "ok"); } catch { toast("ไมค์กำลังทำงานอยู่ กรุณารอสักครู่", "warn"); }
  }
  function apply() { style(); labels(); wireAuth(); renderStudentWords(); }
  ...
  document.addEventListener("click", e => {
    ...
    if (e.target.closest("[data-random-student-clean]")) { e.preventDefault(); e.stopImmediatePropagation(); localStorage.setItem("eng_student_10_start", String(randomStart())); renderStudentWords(); toast("สุ่มคำศัพท์ใหม่ 10 คำเรียบร้อย", "ok"); return; }
    const button = e.target.closest("[data-action]");
    const action = button?.dataset.action || "";
    if (action.includes("record")) { e.preventDefault(); e.stopImmediatePropagation(); startMic(action, button); return; }
    if (role() === "student" && button?.closest("#student")) setTimeout(renderStudentWords, 80);
