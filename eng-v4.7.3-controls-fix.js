(() => {
  ...
  const WORD_COUNT = 10;
  const bank = ["Listen","Speak","Read","Write","Learn","Practice","Message","Question","Answer","Help","Phone","School","Teacher","Student","Family","Friend","Water","Food","House","Market","Travel","Ticket","Hotel","Work","Meeting","Client","Email","Report","Payment","Security","Browser","Lesson","Score","Voice","Meaning","Clear","Daily","Easy","Open","Copy","Book","Pen","Desk","Chair","Table","Room","Door","Window","Morning","Evening","Today","Tomorrow","Money","Shop","Street","City","Country","Airport","Bus","Train","Car","Doctor","Hospital","Bank","Office","Manager","Team","Project","Plan","Time","Date","Week","Month","Year","Price","Order","Service","Problem","Solution","Idea","Note","Picture","Music","Movie","Coffee","Tea","Rice","Fruit","Breakfast","Lunch","Dinner","Happy","Ready","Correct","Polite","Useful","Simple","Important","Beautiful","Careful"];
  ...
  const user = () => { const s = session(); if (!s) return null; if (s.role) return s; return users().find(u => u.id === s.id || u.username === s.username) || s; };
  ...
  function cleanWord(value) {
    ...
    const parts = raw.split(/[-_\s]+/).filter(Boolean).filter(p => /^[a-z]+$/i.test(p));
  ...
  function renderStudentWords(force = false) {
    ...
    if (flow) {
      flow.innerHTML = [["คลังคำศัพท์", "สุ่มคำศัพท์จากฐานข้อมูล และแสดงคำล้วน 10 คำ"], ["Random words", "กดเพื่อเปลี่ยนคำศัพท์ชุดใหม่ทันที"], ["ฝึกต่อเนื่อง", "ฟังเสียง พูดตอบ และทำคะแนนได้ในแต่ละบทเรียน"]].map((x, i) => `<article class="workflow-item"><span class="workflow-number">${i + 1}</span><div><strong>${x[0]}</strong><p>${x[1]}</p></div></article>`).join("");
    }
  ...
  function apply(force = false) {
    if (applying) return;
    applying = true;
    style(); labels(); renderStudentWords(force);
    setTimeout(() => { applying = false; }, 20);
  }
  document.addEventListener("DOMContentLoaded", () => { apply(true); setTimeout(() => apply(true), 500); setTimeout(() => apply(true), 1300); const target = $("student"); if (target) new MutationObserver(() => { if (role() === "student" && !applying) setTimeout(() => apply(false), 60); }).observe(target, { childList: true, subtree: true }); });
  window.addEventListener("pageshow", () => apply(true));
  window.addEventListener("hashchange", () => setTimeout(() => apply(true), 250));
  document.addEventListener("click", event => {
    const random = event.target.closest("[data-random-student-clean]");
    if (random) { event.preventDefault(); event.stopImmediatePropagation(); randomizeWords(); return; }
    const wordButton = event.target.closest("[data-student-clean-word]");
    if (wordButton) { event.preventDefault(); event.stopImmediatePropagation(); speakText(wordButton.textContent || ""); return; }
    const button = event.target.closest("[data-action]");
    const action = button?.dataset.action || "";
    if (!button) return;
    if (action === "random-student") { event.preventDefault(); event.stopImmediatePropagation(); randomizeWords(); return; }
    if (action.includes("record")) { event.preventDefault(); event.stopImmediatePropagation(); recordSpeech(action, button); return; }
    if (action.includes("speak") || action.includes("play") || action.includes("listen")) { event.preventDefault(); event.stopImmediatePropagation(); speakText(targetText(button, action)); return; }
    if (["next-student", "next-word", "pick-student-word"].includes(action)) { setTimeout(() => apply(true), 180); setTimeout(() => apply(true), 700); }
